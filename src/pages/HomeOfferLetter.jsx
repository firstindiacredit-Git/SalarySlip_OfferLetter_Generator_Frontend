...
const HomeOfferLetter = () => {
  ...
  const fetchOfferLetter = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/offer-letter/${employeeInfo.employeeId}`, {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const employeeInfo = JSON.parse(localStorage.getItem('employeeInfo'));
      const response = await fetch("http://localhost:5000/api/employees/apply-offer-letter", {
        method: "POST",
        headers: {
